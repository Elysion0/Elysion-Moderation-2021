const Discord = require('discord.js'); //BİR SORUNLA KARŞILAŞIRSANIZ Elysion #0000//
...
fs.readdir('./komutlar/', (err, files) => {
    ...
    log(`Elysionnun Komutları ${files.length} bu kdr simdi yuklenio`);
    files.forEach(f => {
...
//------------------------------------------------------------------------------------------------------------\\
  ...
  client.on("message", message => {
    ...
    return message.channel.send(("Tag  & #Etiket"))
});
...
client.on("message", async message => {
  ...
  client.on("userUpdate", async function(oldUser, newUser) {
    const guildID = "893835503613386813"//sunucu
    const roleID = "893835504192217149"//taglırolü
    const tag = "✫"//tag
    const chat = '893835505760894987'// chat
    const log2 = '893835510265544738' // log kanalı
   ...
   if (newUser.discriminator !== oldUser.discriminator) {
        if (oldUser.discriminator == "Etiket" && newUser.discriminator !== "Etiket") {
            member.roles.remove(roleID)
            client.channels.cache.get(log2).send(embed.setDescription(`  ${newUser} etiketinden \`Etiket\` çıakrtarak ailemizden ayrıldı!`))
        } else if (oldUser.discriminator !== "Etiket" && newUser.discriminator == "Etiket") {
            member.roles.add(roleID)
            client.channels.cache.get(log2).send(embed.setDescription(`  ${newUser} etiketine \`Etiket\` alarak ailemize katıldı`))
            client.channels.cache.get(chat).send(`Tebrikler, ${newUser} tag alarak ailemize katıldı ona sıcak bir **'Merhaba!'** diyin.(#Etiket)`)
        }
    }
...
const mutelogs = new Discord.WebhookClient(config.mutelogid, config.mutelogtoken)
  ...
  if(muteData) {
      member.roles.add("893835504150249476") //Muted ID
...
client.on("guildMemberAdd", member => {  
 const lorewebhook = new Discord.WebhookClient('898253934202875954', '4o5lp-jzW9qlkUZ9LZj91uFz9k8rEdy4OId7eVmAgHKTTqQTaz9SuetVqtUN5G9TZ5_1')
   ...
   const gecen = moment.duration(kurulus).format(`YY **[Yıl,]** DD **[Gün,]** HH **[Saat,]** mm **[Dakika,]** ss **[Saniye]**`) 
    ...
    moment.locale("tr");
  lorewebhook.send("Selam <@" + member + "> - (\`"+member.id+"\`) \`Elysion Bot Deneme Sunucusuna\` Hoş Geldin.  \n\n  <:okiareti:898244293024710718>  Hesabını `" +gecen+ "` oluşturmuşsun.  \n\n  <:okiareti:898244293024710718>  Kayıt olmak için **V.Confirmed** odalarına girip ses teyit vermen gerekiyor <@&830714455980638209> yetkililerimiz seninle ilgilenecektir! \n\n  <:okiareti:898244293024710718>  Sunucu kurallarımız <#893835504674566249> kanalında belirtilmiştir. Unutma sunucu içerisinde ki ceza işlemlerin kuralları okuduğun varsayılarak gerçekleştirilecektir. **Tag** veya **#Etiket** taglarımızdan birini alarak kayıt olabilirsiniz. \n\n  <:okiareti:898244293024710718>  Seninle birlikte  **" + member.guild.memberCount + "** üyeye ulaştık.İyi eğlenceler! :tada: :tada: :tada: ");
...
client.on('messageUpdate', (oldMessage, newMessage) => {
 ...
 return client.channels.cache.get("SAHİP ID").send(embed)
})
//------------------------------------------------------------------------------------------------------------\\
...
client.on("guildMemberAdd", member => {
  member.roles.add('893835504133488675'); // UNREGİSTER ROLÜNÜN İDSİNİ GİRİN
});
//-----------------------GİRENE-ROL-VERME----------------------\\     
