...
exports.run = async (client, message, args) => {
 ...
 if (!lore1)
    return message.channel.send(
      new Discord.MessageEmbed()
      .setAuthor(message.author.tag, message.author.avatarURL)
      .setDescription(`**Argümanları Eksiksiz Doldur !** \`Örnek\`: .kadın @Etiket/id İsim Yaş`)
      .setColor("2F3136")
      );
  let role = message.guild.roles.cache.get(config.kadın1);
  ...
  if (!nick)
    return message.channel.send(
      new Discord.MessageEmbed()
      .setAuthor(message.author.tag, message.author.avatarURL)
      .setDescription(`**Argümanları Eksiksiz Doldur !** \`Örnek\`: .kadın @Etiket/id İsim `)
      .setColor("2F3136")
    );
    const yas = args[2];
    if (!nick)
...
`);
 ...
 embed.description += `Geçmiş İsimleri Kontrol Etmek İçin .isimler @Etiket/id Yazıp Kontrol Edebilirsin \n\n Elysion ❤️`
