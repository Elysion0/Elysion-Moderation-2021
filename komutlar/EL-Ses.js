...
module.exports.run = async(client, message, args) => {
    ...
    let pub = message.guild.channels.cache.filter(x => x.parentID === "893835506943655984");
    let public = 0;
    for (const [id, voiceChannel] of pub)
    public += voiceChannel.members.size;

    let priv = message.guild.channels.cache.filter(x => x.parentID === "893835507895787520");
    let privkanalları = 0;
    for (const [id, voiceChannel] of priv)
    privkanalları += voiceChannel.members.size;

    let alone = message.guild.channels.cache.filter(x => x.parentID === "893835508185190437");
